...
def process_queries_to_csv(queries, output_filename="musicbrainzngs_resolved_logs.csv"):
    ...
    fieldnames = ["input_artist", "input_title", "source", "resolved_artist", "resolved_title"]
    
    print(f"Processing {len(queries)} tracks using 'musicbrainzngs' -> Saving to '{output_filename}'...\n")
    ...
    with open(output_filename, mode="w", newline="", encoding="utf-8") as csv_file:
        ...
        for idx, query in enumerate(queries):
            ...
            print(f"[{idx+1}/{len(queries)}] Querying -> Artist: '{artist}' | Title: '{title}'")
